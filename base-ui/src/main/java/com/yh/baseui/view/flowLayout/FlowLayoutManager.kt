...
class FlowLayoutManager() : RecyclerView.LayoutManager() {
    ...
    inner class Row {
        ...
        //每一行存储的item
        var views: MutableList<Item> = ArrayList()
    ...
    private fun fillLayout(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        val displayFrame = Rect(paddingLeft, paddingTop + verticalScrollOffset,
                getWidth() - paddingRight, verticalScrollOffset + (getHeight() - paddingBottom))
    ...
    private fun formatAboveRow() {
        ...
        for (i in views.indices) {
            ...
            if (allItemFrames[position].top < row.cuTop + (row.maxHeight - views[i].useHeight) / 2) {
                ...
                frame[allItemFrames[position].left, (row.cuTop + (row.maxHeight - views[i].useHeight) / 2).toInt(), allItemFrames[position].right] = (row.cuTop + (row.maxHeight - views[i].useHeight) / 2 + getDecoratedMeasuredHeight(view)).toInt()
